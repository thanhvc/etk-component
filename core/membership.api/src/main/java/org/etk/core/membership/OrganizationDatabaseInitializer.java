...
public class OrganizationDatabaseInitializer extends BaseComponentPlugin implements
    OrganizationServiceInitializer, ComponentPlugin {

  protected static Logger log = Logger.getLogger(OrganizationDatabaseInitializer.class);
    ...
    for (int i = 0; i < users.size(); i++) {
      ...
      for (int j = 0; j < entry.length; j++) {
        ...
        if (mhandler.findMembershipByUserGroupAndType(data.getUserName(), groupId, membership) == null) {
          ...
          printInfo("    Created membership " + data.getUserName() + ", " + groupId + ", "
              + membership);
        } else {
          printInfo("    Ignored membership " + data.getUserName() + ", " + groupId + ", "
              + membership);
        }
      }
