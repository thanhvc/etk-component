...
import org.etk.kernel.container.management.MetaDataBuilder;
import org.etk.kernel.management.annotations.ImpactType;
import org.etk.kernel.management.spi.ManagedMethodMetaData;
import org.etk.kernel.management.spi.ManagedMethodParameterMetaData;
import org.etk.kernel.management.spi.ManagedParameterMetaData;
import org.etk.kernel.management.spi.ManagedPropertyMetaData;
...
public class ExoMBeanInfoBuilder {
   ...
   * @throws IllegalArgumentException if the class is null or does not contain
   *           meta data
   */
  ...
  private ModelMBeanOperationInfo buildOperationInfo(Method method,
                                                     String description,
                                                     Role role,
    ...
    //
    return new ModelMBeanOperationInfo(operationInfo.getName(),
                                       description,
                                       parameterInfos,
  ...
  public ModelMBeanInfo build() throws IllegalStateException {
    ...
    for (ManagedMethodMetaData methodMD : typeMD.getMethods()) {
      ModelMBeanOperationInfo operationInfo = buildOperationInfo(methodMD.getMethod(),
    ...
    for (ManagedPropertyMetaData propertyMD : typeMD.getProperties()) {
      ...
      if (getter != null) {
        ...
        ModelMBeanOperationInfo operationInfo = buildOperationInfo(getter,
      ...
      if (setter != null) {
        ...
        ModelMBeanOperationInfo operationInfo = buildOperationInfo(setter,
                                                                   propertyMD.getSetterDescription(),
                                                                   Role.SET,
                                                                   blah,
                                                                   ImpactType.IDEMPOTENT_WRITE);
        operations.add(operationInfo);
      }

      //
      try {
        String attributeDescription = propertyMD.getDescription() != null ? propertyMD.getDescription()
                                                                         : ("Managed attribute " + propertyMD.getName());

        //
        ModelMBeanAttributeInfo attributeInfo = new ModelMBeanAttributeInfo(propertyMD.getName(),
    ...
    //
    return new ModelMBeanInfoSupport(typeMD.getType().getName(),
                                     mbeanDescription,
                                     attributeInfos.values()
                                                   .toArray(new ModelMBeanAttributeInfo[attributeInfos.size()]),
