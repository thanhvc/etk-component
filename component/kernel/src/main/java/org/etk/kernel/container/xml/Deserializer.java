...
public class Deserializer {
	...
	/**
	 * Resolve a string value. If the input value is null then the returned
	 * value is null.
	 * 
	 * @param s
	 *            the input value
	...
	public static Integer resolveInteger(String s)
		...
		try {
			...
			throw new IllegalArgumentException("Cannot accept integer value "
					+ s, e);
		}
	}
	 ...
	 * <ol>
	 * <li>Resolve any system property in the input value</li>
	 * <li>Attempt to parse the value using the
	 * {@link Double#parseDouble(String)} method and returns its value.</li>
	 * <li>If the parsing fails then throws an
	 * <code>IllegalArgumentException</code></li>
	 * </ol>
	 * 
			...
			if (c == '$' && state != IN_BRACKET)
				...
				if (start + 2 == i) {
					...
					if (key.equals(Deserializer.EXO_CONTAINER_PROP_NAME)) {
						...
						if (container instanceof ApplicationContainer) {
							// The current container is a portal container
							Object oValue = ((ApplicationContainer) container)
									.getSetting(key.substring(Deserializer.PORTAL_CONTAINER_VARIABLE_PREFIX
													.length()));
